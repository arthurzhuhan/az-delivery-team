...
9. 创建 `.claude/settings.json`（headless 模式权限配置）：
...
**为什么必须有这个文件**：`delivery.sh` 使用 `claude -p`（headless 模式）调用 Claude CLI。非交互模式下无法弹出权限确认框，如果没有预授权，所有写操作会被拒绝，导致 Phase 0 无法写入 Stories、报告等文件。
...
4. 根据项目分析计算推荐的 `--max-rounds` 值（见下方公式）

#### Max-Rounds 推荐公式

每轮 round = Phase 1(实施) → Phase 2(验证) → Phase 3(终审)。NO-GO 时循环回 Phase 1。因此 rounds 数 = 允许多少次 NO-GO 修复循环。

```
推荐 rounds = base + domain_factor + complexity_factor

base = 2（首轮实施 + 至少 1 轮修复）
...
complexity_factor（按项目特征累加，取最高值即可）:
  简单（静态站、CLI 工具、单一 API）              → +0
  中等（标准 Web 全栈、CRUD + 认证）              → +1
  复杂（实时通信/SSE、多外部集成、SDK、复杂状态机） → +2
  极复杂（微服务、多语言 SDK、AI pipeline、合规要求）→ +3
```
...
| 个人博客 | 1 域 | 简单 | 2 |
| SaaS 后台 | 2 域 | 中等 | 3 |
| Verdict (本项目) | 3 域 | 复杂 (SSE+Adapter+CLI+SDK+状态机) | 5 |
| 微服务电商平台 | 4 域 | 极复杂 | 7 |

在输出总结中，用具体项目信息填入公式，给出推荐值和计算过程。同时将推荐值写入 `delivery.json` 的 `maxRounds` 字段。
...
>>> 推荐 max-rounds: [N]（base [X] + 领域 [Y] + 复杂度 [Z]）
    [一句话解释：例如"3 域 + SSE/Adapter/CLI/SDK/状态机 = 复杂"]

>>> 两种交付方式：

  🤖 自动交付（推荐）：
    cd [项目目录]
    ./delivery.sh --max-rounds [N]
    全自动运��� Phase 0(Story 拆解)→1(实施)→2(验证)→3(终审)，NO-GO 自动修复→重验。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 3.3 | 2026-04-13 | (1) Phase 1 批次按全局 priority 排序 — 修复基础 story 因域名字母序排在末尾导致后执行的 bug；(2) Bootstrap story 支持 — 项目初始化类 story 先搭建再写验证测试，跳过严格 TDD 红绿循环；(3) Phase 0 域名约束 — 从 engineer 文件名自动提取允许域列表，注入 prompt 防止 Story 分配到不存在的域 |
| 3.2 | 2026-04-13 | (1) 自动创建 `.claude/settings.json` 授权 headless 模式工具权限 — 修复 `delivery.sh` 因 `claude -p` 无法写文件而失败的问题；(2) Step 7 输出中增加 `--max-rounds` 推荐值，基于领域数量 + 项目复杂度公式计算并写入 `delivery.json` |
...
| 3.0 | 2026-04-12 | 自动交付循环（delivery.sh + delivery.json）：Phase 状态机自动编排；逐 Story 实施循环（fresh context per Story）；跨域并行调度；NO-GO findings 自动转化为 FIX Story；progress.txt 知识沉淀；断点恢复；不收敛检测（failCount + blocked）；手动/自动两种模式共存 |
